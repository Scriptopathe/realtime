...
void initStruct(void) {
    ...
    if (err = rt_mutex_create(&mutexWatchdog, NULL)) {
        rt_printf("Error mutex create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_mutex_create(&mutexPosCompute, NULL)) {
        rt_printf("Error mutex create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_sem_create(&semStartRobot, NULL, 0, S_FIFO)) {
        rt_printf("Error semaphore create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_task_create(&tImageAcquisition, NULL, 0, PRIORITY_TIMAGEACQ, 0)) {
        rt_printf("Error task create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
...
void startTasks() {
    ...
    if (err = rt_task_start(&tImageAcquisition, &acquireImage, NULL)) {
        rt_printf("Error task start: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_task_start(&tArena, &computeArena, NULL)) {
        rt_printf("Error task start: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    if (err = rt_task_start(&tPosition, &computePosition, NULL)) {
        rt_printf("Error task start: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
...
void deleteTasks() {
    ...
    rt_task_delete(&tImageAcquisition);
